const yogaPoses = [
 ...
 // Dropdown element
 const dropdown = document.getElementById("yogaPosesDropdown");

 // Populate dropdown with yoga poses
 ...
 // Function to display pose details
 function displayPoseDetails() {
   const selectedPoseId = dropdown.value;
   const poseDetailsContainer = document.getElementById("poseDetails");
   const selectedPose = yogaPoses.find((pose) => pose.id == selectedPoseId);

   if (selectedPose) {
     // Construct HTML for pose details
       const poseDetailsHTML = `
       <img src="${selectedPose.url_svg}" alt="${selectedPose.english_name} SVG" style="width: 100px; height: 100px; align: center;">
       <p><strong>Sanskrit Name:</strong> ${selectedPose.sanskrit_name}</p>
       <p><strong>Translation Name:</strong> ${selectedPose.translation_name}</p>
       <p><strong>Pose Description:</strong> ${selectedPose.pose_description}</p>
       <p><strong>Pose Benefits:</strong> ${selectedPose.pose_benefits}</p>



     `;

     // Update pose details container
     poseDetailsContainer.innerHTML = poseDetailsHTML;
   } else {
     poseDetailsContainer.innerHTML = ""; // Clear details if no pose is selected
   }
 }